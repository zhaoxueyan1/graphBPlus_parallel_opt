#!/bin/bash
root_dir=`pwd`
work_dir="${root_dir}/build_Serial"
...
do
	log_dir=$root_dir/results_perf/Serial_O0_${bench_lang}/${bench_type}/$((i+1))/
    if [ -d $log_dir ]; then
    ...
    cmd_line="${work_dir}/Serial_graphBplus_O0 ${input_file} 100 ${output_file}"
    cd $work_dir
    time perf stat -r 3 \
